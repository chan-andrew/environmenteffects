...
class LLMTracker {
  ...
  async handleContentScriptData(data) {
    ...
    try {
      console.log('📨 Background: Received content script data:', data);
      
      let tokens = 0;
      let shouldIncrementQuery = false;
      
      if (data.type === 'new_response') {
        tokens = data.responseTokens || 100;
        shouldIncrementQuery = data.isNewQuery || true;
      } else if (data.type === 'streaming_update' || data.type === 'content_update') {
        tokens = data.responseTokens || 0;
        shouldIncrementQuery = false; // Don't increment for streaming updates
      } else if (data.type === 'page_load') {
        tokens = data.responseTokens || 0;
        shouldIncrementQuery = false; // Don't count page loads as queries
      } else {
        tokens = data.responseTokens || data.estimatedTokens || 100;
        shouldIncrementQuery = data.isNewQuery || false;
      }
      
      await this.updateMetrics({
        provider: data.provider || 'Web Interface',
        responseTokens: tokens,
        type: data.type || 'content_script',
        shouldIncrementQuery: shouldIncrementQuery
      });
  ...
  async updateMetrics(data) {
    ...
    // Only increment queries when explicitly told to
    if (data.shouldIncrementQuery) {
    ...
    console.log(`🔥 Background: Updated metrics - ${tokens} tokens from ${data.provider} (${data.type}) query: ${data.shouldIncrementQuery}`);
    console.log(`📊 Background: Daily totals - ${this.dailyData.queries} queries, ${this.dailyData.totalTokens} tokens, ${this.dailyData.carbonGrams.toFixed(1)}g CO₂`);
  }
  ...
  async resetTodayData() {
    ...
    console.log('🔄 Background: Resetting today\'s data');
    ...
    await this.saveDailyData();
    console.log('✅ Background: Data reset complete');
  }
...
chrome.action.onClicked.addListener(async (tab) => {
  ...
  // Check if we're already on a supported LLM site
  ...
  const isOnSupportedSite = supportedSites.some(site => currentUrl.hostname.includes(site));
  
  if (isOnSupportedSite) {
    // Trigger sidebar to show
    try {
      await chrome.storage.local.set({ showSidebar: true });
      console.log('📱 Triggered sidebar to show');
    } catch (error) {
      console.error('❌ Error triggering sidebar:', error);
    }
  } else {
    // Not on a supported site - open ChatGPT
    chrome.tabs.create({ url: 'https://chatgpt.com' });
    console.log('🔗 Opened ChatGPT since not on supported site');
  }
});
